...
public class JsonOptionalSemanticsTest extends JsonContractSupport {
    ...
    void optionalComplex_provided_overridesDefault_andMutatesInnerFields() {
        OptionalComplexInnerFieldMutation data = ok("{\"onnie\":{\"c\":\"k\"}}", OptionalComplexInnerFieldMutation.class);
        assertTrue(data.onnie.isPresent());
    ...
    void providedNilOptional_isIndistinguishableFromMissing_andKeepsDefaultPresent() {
        OptionalLeafWithDefaultPresent data = ok("{\"name\":null}", OptionalLeafWithDefaultPresent.class);
        assertEquals(Optional.empty(), data.name);
    }
