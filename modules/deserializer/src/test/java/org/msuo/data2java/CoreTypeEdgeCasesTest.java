...
public class CoreTypeEdgeCasesTest {
    ...
    void unsupportedParameterizedRaw_reportsTypedError() {
        ...
        ReadResult rr = ObjectMapper
            .adapterFor(weird)
            .read(Path.root(), new JavaScalarDataValue("x"), errors);

        assertFalse(rr.ok);
        assertEquals(1, errors.asList().size());
        assertEquals(
            DataErrorTypes.UnsupportedParameterizedRaw.class,
    ...
    void classReferenceField_resolvesAssignableClass() {
        ClassRefHolder data = ObjectMapper.deserialize(
            wrap(obj("impl", LocalServiceImpl.class.getName())),
            ClassRefHolder.class
        );
        assertEquals(LocalServiceImpl.class, data.impl);
    }

    @Test
    void classReferenceField_rejectsUnknownClass() {
        DataDeserializationException ex = assertThrows(
        ...
        assertEquals(
            DataErrorTypes.ClassRefNotFound.class,
    ...
    void classReferenceField_rejectsNonAssignableClass() {
        ...
        assertEquals(1, ex.getErrors().size());
        assertEquals(
            DataErrorTypes.ClassRefNotAssignable.class,
            ex.getErrors().get(0).getErrorType().getClass()
        );
        assertEquals(
            java.util.Arrays.asList("impl"),
    ...
    void classReferenceField_acceptsSubclassForSuperclassField() {
        ClassRefSuperclassHolder data = ObjectMapper.deserialize(
            wrap(obj("impl", LocalDerived.class.getName())),
            ClassRefSuperclassHolder.class
        );
        assertEquals(LocalDerived.class, data.impl);
    }
    ...
    void classReferenceField_withUnresolvedTypeVariable_reportsUnsupportedType() {
        ...
        assertEquals(
            DataErrorTypes.UnsupportedType.class,
            ex.getErrors().get(0).getErrorType().getClass()
        );
    ...
    void leafCtorInstantiationFailure_reportsCtorCallFailed() {
        ...
        assertEquals(
            DataErrorTypes.CtorCallFailed.class,
            ex.getErrors().get(0).getErrorType().getClass()
        );
    ...
    void nestedNoArgConstructorThrowing_reportsCtorFailed() {
        ...
        assertEquals(
            DataErrorTypes.CtorFailed.class,
            ex.getErrors().get(0).getErrorType().getClass()
        );
    ...
    void nestedAbstractNoArgType_reportsInstantiateFailed() {
        ...
        assertEquals(
            DataErrorTypes.InstantiateFailed.class,
            ex.getErrors().get(0).getErrorType().getClass()
        );
        assertEquals(java.util.Arrays.asList("bad"), ex.getErrors().get(0).getPathSegments());
    }
    ...
    void reflectionMapper_fieldWriteAccessBranch_mapsToFieldSetAccess() {
        DataErrorType error = ReflectionErrorMapper.fieldWriteError(
            new IllegalAccessException("x")
        );
        assertEquals(DataErrorTypes.FieldSetAccess.class, error.getClass());
    }
    ...
    void reflectionMapper_fieldWriteTypeBranch_mapsToFieldSetTypeMismatch() {
        ...
        assertEquals(DataErrorTypes.FieldSetTypeMismatch.class, error.getClass());
    }

    @Test
    void reflectionMapper_fieldReadBranch_mapsToFieldReadAccess() {
        DataErrorType error = ReflectionErrorMapper.fieldReadError(
            new RuntimeException("x")
        );
        assertEquals(DataErrorTypes.FieldReadAccess.class, error.getClass());
    }

    @Test
    void reflectionMapper_invocationCtorBranch_mapsToCtorFailed() {
        DataErrorType error = ReflectionErrorMapper.instantiateError(
            AbstractNoArg.class,
            new InvocationTargetException(new IllegalStateException("boom"))
        );
        assertEquals(DataErrorTypes.CtorFailed.class, error.getClass());
    }
    ...
    private static DataValue wrap(Object value) {
        return new MapListDataValue(value) {
            @Override
            protected DataValue wrap(Object v) {
