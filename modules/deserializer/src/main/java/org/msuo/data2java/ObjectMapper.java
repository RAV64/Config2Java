...
final class ObjectMapper {
    ...
    static <T> T deserialize(DataValue root, Class<T> targetClass) {
        ErrorCollector errors = new ErrorCollector();
        ReadResult rr = readValue(Path.root(), targetClass, root, errors);

        if (errors.hasErrors()) {
            throw new DataDeserializationException(errors.asList());
        }
        if (!rr.ok) {
    ...
    static ReadResult readValue(Path path, Type targetType, DataValue value, ErrorCollector errors) {
