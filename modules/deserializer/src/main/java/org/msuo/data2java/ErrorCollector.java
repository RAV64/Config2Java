...
final class ErrorCollector {
    ...
    void add(Path path, DataErrorType errorType) {
    ...
    List<DataDeserializationException.DataError> asList() {
        List<DataDeserializationException.DataError> out = new ArrayList<>(errors.size());
        for (int i = 0; i < errors.size(); i++) {
            ErrorItem item = errors.get(i);
            out.add(
                new DataDeserializationException.DataError(
    ...
    private static final class ErrorItem {
        ...
        private ErrorItem(Path path, DataErrorType errorType) {
            this.path = path;
