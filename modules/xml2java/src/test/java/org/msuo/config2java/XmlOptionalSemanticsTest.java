...
public class XmlOptionalSemanticsTest extends XmlContractSupport {
    ...
    void missingOptional_withoutDefault_defaultsToEmpty() {
        OptionalLeafNoDefault cfg = ok("<config/>", OptionalLeafNoDefault.class);
    ...
    void missingOptional_keepsDefaultPresent() {
        OptionalLeafWithDefaultPresent cfg = ok("<config/>", OptionalLeafWithDefaultPresent.class);
    ...
    void missingOptional_keepsDefaultEmpty() {
        OptionalLeafWithDefaultEmpty cfg = ok("<config/>", OptionalLeafWithDefaultEmpty.class);
        assertEquals(Optional.empty(), cfg.name);
    }

    @Test
    void providedValidOptionalLeaf_parsesPresent() {
        OptionalLeafNoDefault cfg = ok("<config><n>3</n></config>", OptionalLeafNoDefault.class);
    ...
    void providedInvalidOptionalLeaf_fails_andDoesNotOverwriteDefaultPresent() {
        ConfigDeserializationException ex = fails("<config><name></name></config>", OptionalLeafWithDefaultPresent.class);
        assertSingleError(ex, ConfigErrorTypes.CtorRejected.class, "name");
    }

    @Test
    void providedInvalidOptionalLeaf_fails_atOptionalPath() {
        ConfigDeserializationException ex = fails("<config><n>0</n></config>", OptionalLeafNoDefault.class);
        assertSingleError(ex, ConfigErrorTypes.CtorRejected.class, "n");
    }

    @Test
    void optionalComplex_missing_withoutDefault_defaultsToEmpty() {
        OptionalComplexNoDefault cfg = ok("<config/>", OptionalComplexNoDefault.class);
        assertEquals(Optional.empty(), cfg.onnie);
    }

    @Test
    void optionalComplex_missing_keepsDefaultPresent() {
        OptionalComplexWithDefaultPresent cfg = ok("<config/>", OptionalComplexWithDefaultPresent.class);
        assertTrue(cfg.onnie.isPresent());
        assertEquals("c", cfg.onnie.get().c);
        assertEquals(Integer.valueOf(5), cfg.onnie.get().d);
    }
    ...
    void optionalComplex_provided_overridesDefault_andMutatesInnerFields() {
        OptionalComplexInnerFieldMutation cfg = ok("<config><onnie><c>k</c></onnie></config>", OptionalComplexInnerFieldMutation.class);
        assertTrue(cfg.onnie.isPresent());
    ...
    void optionalComplex_providedEmptyTable_isPresent_andKeepsInnerDefaults() {
        OptionalComplexNoDefault cfg = ok("<config><onnie><dummy/></onnie></config>", OptionalComplexNoDefault.class);
    ...
    void optionalComplex_providedButInnerCannotInstantiate_fails() {
        ConfigDeserializationException ex = fails("<config><bad><x>ok</x></bad></config>", OptionalBadInnerNoNoArg.class);
