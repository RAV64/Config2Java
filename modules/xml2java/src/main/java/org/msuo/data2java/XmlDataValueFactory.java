...
final class XmlDataValueFactory {

    private XmlDataValueFactory() {}

    static DataValue rootValue(Element root) {
        return new XmlElementValue(root, false);
    }

    private static DataValue valueForElement(Element element) {
    ...
    private static final class XmlElementValue implements DataValue {
    ...
    private static final class XmlElementListValue implements DataValue {
    ...
    private static final class XmlElementTable implements DataTable {
        ...
        public DataValue getField(String key) {
            if (attributes.containsKey(key)) {
                return new JavaScalarDataValue(attributes.get(key));
            }
            ...
            if ("text".equals(key) && structuredText != null) {
                return new JavaScalarDataValue(coerceScalar(structuredText));
            }
                ...
                public DataEntry next() {
                    if (attrsIt.hasNext()) {
                        ...
                        return DataEntry.of(
                            ...
                            new JavaScalarDataValue(e.getValue()),
                            e.getKey()
                        );
                    }
                    if (childIt.hasNext()) {
                        Map.Entry<String, List<Element>> e = childIt.next();
                        DataValue value = e.getValue().size() == 1
                    ...
                    textConsumed = true;
                    return DataEntry.of(
                        new JavaScalarDataValue("text"),
                        new JavaScalarDataValue(coerceScalar(structuredText)),
                        "text"
                    );
                }
            };
    ...
    private static final class XmlElementListTable implements DataTable {
        ...
        public Iterable<DataEntry> entries() {
            return () -> new Iterator<DataEntry>() {
                ...
                public DataEntry next() {
                    int key = index;
                    DataValue value = valueForElement(elements.get(index - 1));
                    index++;
                    return DataEntry.of(
