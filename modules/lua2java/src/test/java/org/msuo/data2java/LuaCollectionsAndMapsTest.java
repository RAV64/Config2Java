...
public class LuaCollectionsAndMapsTest extends LuaContractSupport {
    ...
    void listOfLeafTypes_supported() {
        ListOfLeaf data = ok("return { tags = { 'a', 'b' } }", ListOfLeaf.class);
    ...
    void setOfLeafTypes_supported_andDeduplicates() {
        SetOfLeaf data = ok("return { tags = { 'a', 'b', 'a' } }", SetOfLeaf.class);
        assertEquals(2, data.tags.size());
    }

    @Test
    void mapOfLeafTypes_supported() {
        MapOfLeaf data = ok("return { limits = { foo = 1, bar = 2 } }", MapOfLeaf.class);
        assertEquals(2, data.limits.size());
    }

    @Test
    void mapKeyWrongType_reportsError_atKeyPathWithBraces() {
        DataDeserializationException ex = fails("return { limits = { foo = 1 } }", MapKeyWrongType.class);
        assertSingleError(ex, DataErrorTypes.NoOneArgCtor.class, "limits", "{foo}");
    }

    @Test
    void listCanContainComplexTypes() {
        ListOfComplex data = ok("return { items = { { name = 'a' }, { name = 'b' } } }", ListOfComplex.class);
    ...
    void defaultMap_isKeptWhenKeyMissing() {
        DefaultMapKept data = ok("return {}", DefaultMapKept.class);
        assertEquals(1, data.limits.size());
    }

    @Test
    void listElementWrongType_reportsError_atElementPath() {
        DataDeserializationException ex = fails("return { tags = { 1 } }", ListElementWrongType.class);
        assertSingleError(ex, DataErrorTypes.NoOneArgCtor.class, "tags", "[1]");
    }

    @Test
    void mapBadEntry_reportsError() {
        DataDeserializationException ex = fails("return { limits = { ok = 1, bad = 0 } }", MapHasBadEntry.class);
        assertSingleError(ex, DataErrorTypes.CtorRejected.class, "limits", "[bad]");
    }

    @Test
    void nestedGenericsInMap_areSupported() {
        NestedGenericsMapValue data = ok("return { bad = { foo = { 'a', 'b' } } }", NestedGenericsMapValue.class);
