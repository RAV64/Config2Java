...
public class LuaOptionalSemanticsTest extends LuaContractSupport {
    ...
    void providedNilOptional_isIndistinguishableFromMissing_andKeepsDefaultPresent() {
        OptionalLeafWithDefaultPresent data = ok("return { name = nil }", OptionalLeafWithDefaultPresent.class);
        assertTrue(data.name.isPresent());
        assertEquals("default", data.name.get().value);
    }
